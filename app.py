...
def parse_ytd_from_paystub_text(text):
    ...
    # Pull the bottom totals row only. The paystub has this exact section:
    # YTD Gross | YTD Deductions | YTD Net Pay | Current Total | Current Deductions | Net Pay
    # The first 3 amounts in that row are the saved YTD values. The last 3 are current-check values.
    # This avoids accidentally importing Current Total as YTD Gross or Net Pay as YTD Net.
    ytd_gross = ytd_deductions = ytd_net = 0.0
    money_re = r"[-+]?\(?\$?\d[\d,]*(?:\.\d{2})?\)?"

    compact_full = re.sub(r"\s+", " ", full)
    totals_match = re.search(
        r"YTD\s+Gross\s+YTD\s+Deductions\s+YTD\s+Net\s+Pay\s+Current\s+Total\s+Current\s+Deductions\s+Net\s+Pay\s+"
        + rf"({money_re})\s+({money_re})\s+({money_re})\s+({money_re})\s+({money_re})\s+({money_re})",
        compact_full,
        re.IGNORECASE,
    )
    if totals_match:
        ytd_gross = safe_num(totals_match.group(1))
        ytd_deductions = safe_num(totals_match.group(2))
        ytd_net = safe_num(totals_match.group(3))
    else:
        # Fallback for PDFs where text extraction splits headers/amounts across lines.
        for idx, line in enumerate(lines):
            lower_line = line.lower()
            if "ytd gross" in lower_line and "ytd deductions" in lower_line and "ytd net" in lower_line:
                window = " ".join(lines[idx: idx + 8])
                nums = re.findall(money_re, window)
                # If current totals are also present, there should be 6 amounts; first 3 are YTD.
                # If only 3 amounts are extracted, treat those as the YTD row.
                if len(nums) >= 3:
